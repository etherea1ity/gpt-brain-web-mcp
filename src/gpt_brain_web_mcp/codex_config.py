...
def block(python_path: str, home: str, headless: bool = True) -> str:
    env = f'{{ "GPT_BRAIN_BACKEND" = "web-chatgpt", "GPT_BRAIN_DEFAULT_TIER" = "thinking_heavy", "GPT_BRAIN_ALLOW_PRO_DEFAULT" = "false", "GPT_BRAIN_BROWSER_HEADLESS" = "{str(headless).lower()}", "GPT_BRAIN_HOME" = "{home}" }}'
    return f'[mcp_servers.gpt-brain-web]\ncommand = "{python_path}"\nargs = ["-m", "gpt_brain_web_mcp.server"]\nenv = {env}\n'


def merge_config(path: str | Path, python_path: str, home: str, dry_run: bool = False, uninstall: bool = False, headless: bool = True) -> str:
    ...
    if uninstall:
        ...
        new_block = block(python_path, home, headless); nxt = text.find("\n[", start+1) if start >= 0 else -1
