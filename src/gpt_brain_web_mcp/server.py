...
def create_mcp(service=None):
    ...
    @mcp.tool()
    async def list_projects(limit: int = 50) -> dict[str, Any]: return await runner.run("list_projects", limit)
    @mcp.tool()
    async def open_project(project: str) -> dict[str, Any]: return await runner.run("open_project", project)
    @mcp.tool()
    async def create_project(project: str, confirm: bool = False) -> dict[str, Any]: return await runner.run("create_project", project, confirm)
    @mcp.tool()
    async def start_project_conversation(project: str, question: str | None = None, tier: str | None = None, cleanup_remote: bool = False) -> dict[str, Any]: return await runner.run("start_project_conversation", project, question, tier, cleanup_remote)
    @mcp.tool()
    async def delete_remote_project(project: str, confirm: bool = False, confirm_name: str | None = None, purge_local: bool = False) -> dict[str, Any]: return await runner.run("delete_remote_project", project, confirm, confirm_name, purge_local)
    @mcp.tool()
